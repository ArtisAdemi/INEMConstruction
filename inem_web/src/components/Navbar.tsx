...
export const Navbar: React.FC = () => {
    ...
    return (
        <div className={`navbar-wrapper w-full ${isAtTop ? "bg-[rgba(255,255,255,0)]" : "bg-[rgba(255,255,255,1)]"} flex justify-between px-[8%] xl:px-[16%] py-4 transition-all duration-500`}>
            <div className="">
                <img className="w-[40%] cursor-pointer" src="#" alt="logo image" onClick={() => redirect("/")} />
            </div>
            <div className="gap-x-10 items-center hidden xl:flex">
                <h2 className={`cursor-pointer hover:text-[#0B63E5] hover:duration-300 duration-300 ${isActive("/") ? "font-bold text-[#0B63E5]" : "text-black"}`} onClick={() => redirect("/")}>
                    Home
                </h2>
                <h2 className={`cursor-pointer hover:text-[#0B63E5] hover:duration-300 duration-300 ${isActive("/projects") ? "font-bold text-[#0B63E5]" : "text-black"}`} onClick={() => redirect("/projects")}>
                    Projects
                </h2>
                <h2 className={`cursor-pointer hover:text-[#0B63E5] hover:duration-300 duration-300 ${isActive("/services") ? "font-bold text-[#0B63E5]" : "text-black"}`} onClick={() => redirect("/services")}>
                    Services
                </h2>
                <h2 className={`cursor-pointer hover:text-[#0B63E5] hover:duration-300 duration-300 ${isActive("/about") ? "font-bold text-[#0B63E5]" : "text-black"}`} onClick={() => redirect("/about")}>
                    About Us
                </h2>
            </div>
            <div className="items-center justify-center hidden xl:flex h-full">
                <button className={`bg-[#0B63E5] mt-3 h-1/2 rounded-md px-[20px] duration-300 py-[10px] text-[#FFFFFF] font-medium hover:cursor-pointer hover:opacity-80 transform hover:scale-105 ${isActive("/contact") ? "font-bold text-[#0B63E5]" : ""}`} onClick={() => redirect("/contact")}>
                    Contact
                </button>
            </div>
